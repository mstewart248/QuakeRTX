...
extern cvar_t d3d9_modelnormals;

static void GLMesh_BuildD3D9AliasBuffer (qmodel_t *m, aliashdr_t *hdr);
static void GLMesh_BuildD3D9BlendedBuffer (qmodel_t *m, aliashdr_t *hdr);
static void GLMesh_BuildD3D9SkeletalPoses (qmodel_t *m, aliashdr_t *hdr);
...
void GLMesh_LoadVertexBuffer (qmodel_t *m, aliashdr_t *mainhdr)
{
	...
	//NB: mainhdr, not hdr -- the surface walk above leaves hdr as NULL
	if (D3D9_Active())
	{
		if (mainhdr->poseverttype == PV_IQM)
		{
			//hardware skinning if the skeleton fits the blend palette, which
			//gives one hash for the whole model; otherwise skin every pose now
			if (mainhdr->numbones <= D3D9_MaxBones())
				GLMesh_BuildD3D9BlendedBuffer (m, mainhdr);
			else
				GLMesh_BuildD3D9SkeletalPoses (m, mainhdr);
		}
		else
			GLMesh_BuildD3D9AliasBuffer (m, mainhdr);
	}
...
/*
================
GLMesh_BuildD3D9AliasBuffer

Expands every keyframe pose to float positions once, into a single immutable
D3D9 vertex buffer. Drawing a pose is then just a stream offset, so the vertex
data is identical every time that pose appears and Remix can hash it.

Positions in a .mdl are bytes in the model's own compressed space, so they get
the scale/scale_origin applied here rather than through a matrix -- keeping the
world transform free for entity placement.

Only PV_QUAKE1 (classic .mdl) is handled here. Skeletal models take an entirely
different route -- see GLMesh_BuildD3D9SkeletalBuffer -- and MD3 falls back to
not drawing rather than rendering wrongly.
================
*/
static void GLMesh_BuildD3D9AliasBuffer (qmodel_t *m, aliashdr_t *hdr)
{
	int		pose, i, numposes, numverts, stride;
	qboolean	withnormals = d3d9_modelnormals.value ? true : false;
	trivertx_t	*verts;
	aliasmesh_t	*desc;
	...
	numverts = hdr->numverts_vbo;
	numposes = hdr->nummorphposes;

	if (numverts <= 0 || numposes <= 0 || hdr->numindexes <= 0)
		return;

	verts   = (trivertx_t *)((byte *)hdr + hdr->vertexes);
	desc    = (aliasmesh_t *)((byte *)hdr + hdr->meshdesc);
	indices = (unsigned short *)((byte *)hdr + hdr->indexes);

	//johnfitz -- padded skins: the texture may be larger than the skin
	hscale = (float)hdr->skinwidth / (float)TexMgr_PadConditional(hdr->skinwidth);
	vscale = (float)hdr->skinheight / (float)TexMgr_PadConditional(hdr->skinheight);

	stride = withnormals ? 8 : 5;

	buf = (float *) malloc ((size_t)numverts * numposes * stride * sizeof(float));
	if (!buf)
		return;
	...
	{
		//the trivertx array is already in vbo vertex order, so no indirection
		trivertx_t *pv = verts + (size_t)pose * numverts;

		for (i = 0; i < numverts; i++)
		{
			*out++ = pv[i].v[0] * hdr->scale[0] + hdr->scale_origin[0];
			*out++ = pv[i].v[1] * hdr->scale[1] + hdr->scale_origin[1];
			*out++ = pv[i].v[2] * hdr->scale[2] + hdr->scale_origin[2];

			//the normal the model has always carried, one of the 162 Quake
			//quantises them to
			if (withnormals)
			{
				const float *n = r_avertexnormals[pv[i].lightnormalindex];
				*out++ = n[0];
				*out++ = n[1];
				*out++ = n[2];
			}

			//desc[].st are raw texel coords, not normalised. Same formula the
			//GL path uses, including the padded-skin correction.
			*out++ = hscale * ((float) desc[i].st[0] + 0.5f) / (float) hdr->skinwidth;
			*out++ = vscale * ((float) desc[i].st[1] + 0.5f) / (float) hdr->skinheight;
		}
	}

	D3D9_CreateAliasBuffer (&m->d3dalias, buf, numverts, numposes, indices, hdr->numindexes, withnormals);
...
/*
================
GLMesh_BuildD3D9BlendedBuffer

Builds the one static bind-pose buffer a hardware-skinned PV_IQM model needs:
position, three blend weights, four packed bone indices, texcoord. Animation
never touches it -- it arrives as bone matrices at draw time -- so a single
geometry hash covers every frame the model has, which is the opposite of the
pose-per-buffer trick the .mdl path has to play.

The loader has already reduced each vertex to at most four influences and
normalised them, so the weights drop straight in and the fourth is implied.

Only usable when the whole skeleton fits the device's blend palette, which is
smaller than a rerelease monster needs on the runtimes tested so far -- see
GLMesh_BuildD3D9SkeletalPoses for what happens then.

Only the first surface is built. Every model in the 2021 rerelease is a single
mesh; a multi-mesh md5 would need one buffer per surface and one draw each.
================
*/
static void GLMesh_BuildD3D9BlendedBuffer (qmodel_t *m, aliashdr_t *hdr)
{
	...
	if (numverts <= 0 || hdr->numindexes <= 0 || hdr->numbones <= 0)
		return;

	if (hdr->nextsurface)
		Con_DWarning ("%s: only the first of its surfaces will be drawn under -d3d9\n", m->name);

	in      = (const iqmvert_t *)((byte *)hdr + hdr->vertexes);
	indices = (unsigned short *)((byte *)hdr + hdr->indexes);

	buf = (float *) malloc ((size_t)numverts * 9 * sizeof(float));
	if (!buf)
		return;

	out = buf;
	for (i = 0; i < numverts; i++, in++)
	{
		*out++ = in->xyz[0];
		*out++ = in->xyz[1];
		*out++ = in->xyz[2];

		//three weights; the fourth is 1 minus these, which is exact because
		//MD5_BakeInfluences normalised them
		*out++ = in->weight[0];
		*out++ = in->weight[1];
		*out++ = in->weight[2];

		//four bone indices in one dword, byte n feeding weight n
		packed =  (unsigned int)in->idx[0]
			| ((unsigned int)in->idx[1] << 8)
			| ((unsigned int)in->idx[2] << 16)
			| ((unsigned int)in->idx[3] << 24);
		memcpy (out++, &packed, sizeof(packed));

		//md5 texcoords are already normalised, and the skins are not padded
		*out++ = in->st[0];
		*out++ = in->st[1];
	}

	D3D9_CreateSkeletalBuffer (&m->d3dskel, buf, numverts, indices, hdr->numindexes, hdr->numbones);
...
Skins numverts bind-pose vertices by the given bone matrices into out, as
xyz + st per vertex.

skin[] is one matrix per bone, absolute and already multiplied by the inverse
bind pose -- what R_SetupAliasFrame leaves in lerpdata.bonestate, and what the
pose expansion below builds for each keyframe. Doing both through here is what
keeps a lerped frame consistent with the baked ones.
================
*/
void GLMesh_SkinVerts (const iqmvert_t *in, int numverts, const bonepose_t *skin, float *out, qboolean withnormals)
{
	int i, j, k;
	...
	for (i = 0; i < numverts; i++, in++)
	{
		...
		for (k = 0; k < 4; k++)
		{
			...
			mat = skin[in->idx[k]].mat;
			for (j = 0; j < 3; j++)
				xyz[j] += w * (mat[j*4+0]*in->xyz[0] + mat[j*4+1]*in->xyz[1]
						+ mat[j*4+2]*in->xyz[2] + mat[j*4+3]);

			//the normal follows the bone's rotation only, so no translation
			if (withnormals)
				for (j = 0; j < 3; j++)
					norm[j] += w * (mat[j*4+0]*in->norm[0] + mat[j*4+1]*in->norm[1]
							+ mat[j*4+2]*in->norm[2]);
		}
		...
		*out++ = xyz[2];
		...
		if (withnormals)
		{
			//blending two rotations shortens the result; renormalise
			float len = sqrt (norm[0]*norm[0] + norm[1]*norm[1] + norm[2]*norm[2]);
			if (len > 0)
				len = 1.0f / len;
			else
				norm[2] = 1, len = 1;	//degenerate: point it somewhere valid
			*out++ = norm[0] * len;
			*out++ = norm[1] * len;
			*out++ = norm[2] * len;
		}
...
/*
================
GLMesh_BuildD3D9SkeletalPoses

The fallback for skeletons too big for the device's blend palette, which is
every rerelease monster on the Remix runtime: its fixed function pipeline
offers nine matrices and a shambler has thirty-six.

So do the skinning here instead, once, for every keyframe the animation has,
and hand the result to the ordinary .mdl path as an ordinary pose-expanded
buffer. The work is the same either way -- the only question is whether it
happens per frame on the GPU or once at load -- and doing it at load keeps the
one property that matters: the vertex data for a given pose is written once and
never rewritten, so its hash is stable and Remix can anchor to it.

The cost is memory and hash count. A pose is numverts * 20 bytes, so a 147
frame ogre is under three megabytes, and each pose is a separate mesh to Remix
exactly as each frame of a .mdl already is. rtx.geometryAssetHashRuleString is
the usual answer to that: drop positions from the asset hash and every pose of
a model resolves to one replaceable asset.

Bones arrive as parent-relative keyframes, so each pose is walked in hierarchy
order to make it absolute and then multiplied by the inverse bind pose, which
is what turns a bind-pose vertex into an animated one. Same maths the GL
skeletal path does per frame, just hoisted.
================
*/
static void GLMesh_BuildD3D9SkeletalPoses (qmodel_t *m, aliashdr_t *hdr)
{
	int		i, j, b, pose, numposes, numverts, stride;
	qboolean	withnormals = d3d9_modelnormals.value ? true : false;
	const iqmvert_t	*verts;
	const boneinfo_t *bi;
	const bonepose_t *keyframes;
	unsigned short	*indices;
	bonepose_t	*abs, *skin;
	float		*buf, *out;
	size_t		bytes;
	...
	numverts = hdr->numverts_vbo;
	numposes = hdr->numboneposes;
	if (numposes < 1)
		numposes = 1;	//a model with no animation still has its bind pose
	...
	//a 64mb ceiling is far above any real model and well below anything that
	//would embarrass the device
	stride = withnormals ? 8 : 5;
	bytes = (size_t)numverts * numposes * stride * sizeof(float);
	if (bytes > 64u*1024u*1024u)
	{
		Con_Warning ("%s: %d poses of %d verts is too much to expand\n", m->name, numposes, numverts);
		return;
	}

	verts     = (const iqmvert_t *)((byte *)hdr + hdr->vertexes);
	indices   = (unsigned short *)((byte *)hdr + hdr->indexes);
	bi        = (const boneinfo_t *)((byte *)hdr + hdr->boneinfo);
	keyframes = (const bonepose_t *)((byte *)hdr + hdr->boneposedata);

	abs  = (bonepose_t *) malloc ((size_t)hdr->numbones * sizeof(*abs) * 2);
	...
	skin = abs + hdr->numbones;

	out = buf;
	for (pose = 0; pose < numposes; pose++)
	{
		const bonepose_t *frame = hdr->numboneposes ? keyframes + (size_t)pose * hdr->numbones : NULL;

		for (b = 0; b < hdr->numbones; b++)
		{
			if (!frame)
			{	//no animation at all: the bind pose is the only pose, and
				//skinning it by identity leaves the vertices where they are
				memset (skin[b].mat, 0, sizeof(skin[b].mat));
				skin[b].mat[0] = skin[b].mat[5] = skin[b].mat[10] = 1;
				continue;
			}

			if (bi[b].parent < 0)
				memcpy (abs[b].mat, frame[b].mat, sizeof(abs[b].mat));
			else	//parents always come first, so this is already absolute
				R_ConcatTransforms ((void*)abs[bi[b].parent].mat, (void*)frame[b].mat, (void*)abs[b].mat);

			R_ConcatTransforms ((void*)abs[b].mat, (void*)bi[b].inverse.mat, (void*)skin[b].mat);
		}

		GLMesh_SkinVerts (verts, numverts, skin, out, withnormals);
		out += (size_t)numverts * stride;
	}

	/*
		Widen the model's bounds to what the animation actually reaches.

		Mod_CalcAliasBounds measures a skeletal model in its bind pose, since
		that is the only vertex data there is until something skins it -- but
		culling then works off a box the animation leaves. A shambler swings
		65 units outside its bind pose, so it vanishes while a good part of it
		is still on screen.

		We have just skinned every pose, so take the real extent. Only ever
		widen: this decides visibility, and being generous costs a draw call
		while being wrong costs a monster.
	*/
	{
		vec3_t	mins, maxs;
		float	radius = 0, yawradius = 0;

		mins[0] = mins[1] = mins[2] =  FLT_MAX;
		maxs[0] = maxs[1] = maxs[2] = -FLT_MAX;

		for (i = 0; i < numverts*numposes; i++)
		{
			const float *v = buf + (size_t)i*stride;
			float dist;
		...
		for (j = 0; j < 3; j++)
		{
			m->mins[j] = q_min(m->mins[j], mins[j]);
			m->maxs[j] = q_max(m->maxs[j], maxs[j]);
			m->rmins[j] = q_min(m->rmins[j], -radius);
			m->rmaxs[j] = q_max(m->rmaxs[j],  radius);
		}
		for (j = 0; j < 2; j++)
		{
			m->ymins[j] = q_min(m->ymins[j], -yawradius);
			m->ymaxs[j] = q_max(m->ymaxs[j],  yawradius);
		}
		m->ymins[2] = q_min(m->ymins[2], mins[2]);
		m->ymaxs[2] = q_max(m->ymaxs[2], maxs[2]);

		Con_DPrintf ("%s: %d poses skinned, %.0f %.0f %.0f .. %.0f %.0f %.0f\n", m->name,
				numposes, mins[0], mins[1], mins[2], maxs[0], maxs[1], maxs[2]);
	}
...
void Mod_LoadIQMModel (qmodel_t *mod, const void *buffer)
{
	...
	//after the bounds, for the same reason as the md5 loader
	GLMesh_LoadVertexBuffer (mod, outhdr);
...
void Mod_LoadMD5MeshModel (qmodel_t *mod, const void *buffer)
{
	...
	//after the bounds, not before: this measures the model in its bind pose,
	//and building the buffers is what skins every keyframe and finds out how
	//far the animation actually reaches
	GLMesh_LoadVertexBuffer (mod, outhdr);
