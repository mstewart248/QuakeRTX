...
static qboolean VID_SetMode (int width, int height, int refreshrate, int bpp, qboolean fullscreen)
{
	...
	if (!draw_context)
	{
		//a D3D9 device wants a plain window. SDL_WINDOW_OPENGL would have SDL
		//choose a GL pixel format for it, which we neither need nor want.
		flags = D3D9_Requested() ? SDL_WINDOW_HIDDEN : (SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN);
	...
	if (D3D9_Requested())
	{
		...
		if (!SDL_GetWindowWMInfo (draw_context, &wminfo))
			Sys_Error ("Couldn't get window handle for D3D9: %s", SDL_GetError());

		SDL_GetWindowSize (draw_context, &vid.width, &vid.height);

		if (D3D9_Active())
			D3D9_Resize (vid.width, vid.height, fullscreen);
		else if (!D3D9_Init (wminfo.info.win.window, vid.width, vid.height, fullscreen))
			Sys_Error ("Couldn't create D3D9 device");

		gl_swap_control = false;	//D3D9 presentation interval is set with the swapchain
	}
	else
#endif
	{
...
static void GL_CheckExtensions (void)
{
	...
	if (gl_version_major > 4 || (gl_version_major == 4 && gl_version_minor >= 4) || GL_ParseExtensionList(gl_extensions, "GL_ARB_buffer_storage"))
	{
		...
		{	//callers gate on these pointers alone, but the PBO lightmap path also uses
			//GL_GenBuffersFunc etc, which stay NULL without vbo support (-novbo, or any
			//driver missing ARB_vertex_buffer_object). Clear them so those checks hold.
...
{
	if (D3D9_Active())
		return;		//no GL context exists under -d3d9
...
{
	if (D3D9_Active())
	{	//no GL context exists under -d3d9, so GL_CheckExtensions never runs and
		//the limits it would have queried have to come from D3D9 caps instead.
		//gl_hardware_maxsize is load-bearing: TexMgr_SafeTextureSize clamps
		//every dimension to it, so leaving it at zero makes TexMgr_LoadImage32
		//mip each texture down to 0x0 and every CreateTexture fail.
		gl_hardware_maxsize = D3D9_MaxTextureSize ();
		LMBLOCK_WIDTH = q_min(gl_hardware_maxsize, 512);
		LMBLOCK_HEIGHT = q_min(gl_hardware_maxsize, 16384);
		Con_Printf ("D3D9: max texture size %d\n", gl_hardware_maxsize);

		//same story: gl_max_anisotropy would otherwise stay at zero and
		//TexMgr_Anisotropy_f would clamp the cvar to it, so anisotropic
		//filtering could never be switched on and every surface seen at an
		//angle stays blurred until you walk onto it
		gl_max_anisotropy = (float) D3D9_MaxAnisotropy ();
		gl_anisotropy_able = (gl_max_anisotropy >= 2);
		Con_Printf ("D3D9: max anisotropy %g\n", gl_max_anisotropy);
		return;
	}
...
{
	if (D3D9_Active())
	{	//always present: BeginFrame opened a scene, and leaving it open when
		//the update is skipped would desync the device.
		D3D9_EndFrame ();
		return;
	}
...
void	VID_Shutdown (void)
{
	if (vid_initialized)
	{
		...
		//release the device before the window it was created on goes away
		D3D9_Shutdown ();
#if defined(USE_SDL2)
...
#endif	//SDL 1.2 has no SDL_GetDefaultCursor, so just leave the current cursor in place.
	}
}
...
#endif	//SDL 1.2 has no SDL_BlitScaled; the cursor is used at its native size.
		}
	}
...
#else
		vm->cursorhandle = NULL;	//SDL 1.2 has no colour cursors, so QC cursor images are ignored
#endif
		SDL_FreeSurface(surf);
	}
	else
