...
//d3d9_nocull latched for the frame -- see R_SetupView and R_CullBox
int			r_nocull;
...
qboolean R_CullBox (vec3_t emins, vec3_t emaxs)
{
	...
	/*
		Under -d3d9 the frustum is not the boundary of the scene, only of the
		image. Off-screen geometry still has to reach Remix or it cannot appear
		in a reflection or contribute a bounce. Resolved once per frame in
		R_SetupView rather than read from the cvar here, since this is on the
		hot path for every leaf, surface and entity.
	*/
	if (r_nocull)
		return false;
...
void R_SetupGL (void)
{
	...
	if (D3D9_Active())
	{
		...
		D3D9_SetViewport (glx + r_refdef.vrect.x,
				gly + glheight - r_refdef.vrect.y - r_refdef.vrect.height,
				r_refdef.vrect.width / scale,
				r_refdef.vrect.height / scale);

		//reuse the engine's own matrices rather than rebuilding them
		Matrix4_ProjectionMatrix (r_fovx, r_fovy, NEARCLIP, gl_farclip.value, false, frustum_skew, 0, mat);
		D3D9_SetProjectionFromGL (mat);

		Matrix4_ViewMatrix (r_refdef.viewangles, r_refdef.vieworg, mat);
		D3D9_SetViewFromGL (mat);

		D3D9_Set3DState (gl_cull.value != 0);

		//hand Quake's map lights to Remix as fixed-function lights
		{
			extern void R_D3D9_SubmitMapLights (void);
...
// current viewleaf
	...
	/*
		Latch how much of the level to submit for the whole frame. R_CullBox and
		R_BackFaceCull are called thousands of times per frame from the surface
		walk, so they read this rather than the cvar, and it cannot change
		halfway through a frame and leave the texture chains inconsistent.
	*/
	{
		extern cvar_t d3d9_nocull;
		r_nocull = D3D9_Active() ? (int)d3d9_nocull.value : 0;
	}
