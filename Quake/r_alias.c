...
void R_SetupAliasFrame (aliashdr_t *paliashdr, entity_t *e, lerpdata_t *lerpdata)
{
	...
	else
	{
		...
		/*
			...
			d3d9_lerpmodels counts as asking for lerping too. The -d3d9
			backend used to need r_lerpmodels 0 -- lerping meant writing
			blended vertices every frame, which is exactly what it could not
			do -- and a config carrying that setting would otherwise leave
			these models stepping at 10Hz with no way to fix it short of
			knowing about both cvars.
		*/
		if ((r_lerpmodels.value || (D3D9_Active() && d3d9_lerpmodels.value)) &&
...
/*
=================
R_BuildLerpedAliasVerts

Produces the frame the engine actually wants shown -- between two keyframes,
not on one -- as xyz + st per vertex, for the -d3d9 backend to draw directly.

Skeletal models interpolate their bones, which R_SetupAliasFrame has already
done by the time we get here, so skinning by lerpdata.bonestate is the whole
job. Morph models interpolate the vertices themselves, between pose1 and pose2.

Returns NULL for anything it cannot interpolate, which the caller reads as
"draw the keyframe out of the static buffer instead".
=================
*/
static const float *R_BuildLerpedAliasVerts (aliashdr_t *hdr, lerpdata_t *lerpdata, int *outnumverts, qboolean withnormals)
{
	static float	*scratch = NULL;
	static int	scratchsize = 0;
	int		numverts = hdr->numverts_vbo;
	int		stride = withnormals ? 8 : 5;

	if (numverts <= 0 || hdr->numindexes <= 0)
		return NULL;

	if (numverts * stride > scratchsize)
	{
		float *grown = (float *) realloc (scratch, (size_t)numverts * stride * sizeof(float));
		if (!grown)
			return NULL;
		scratch = grown;
		scratchsize = numverts * stride;
	}
	...
	*outnumverts = numverts;
	...
	{
		const iqmvert_t *verts = (const iqmvert_t *)((byte *)hdr + hdr->vertexes);

		if (!lerpdata->bonestate)
			return NULL;	//nothing animating it

		GLMesh_SkinVerts (verts, numverts, lerpdata->bonestate, scratch, withnormals);
		return scratch;
	}

	if (hdr->poseverttype == PV_QUAKE1)
	{
		const trivertx_t *verts = (const trivertx_t *)((byte *)hdr + hdr->vertexes);
		const aliasmesh_t *desc = (const aliasmesh_t *)((byte *)hdr + hdr->meshdesc);
		const trivertx_t *v1, *v2;
		float		blend = lerpdata->blend;
		float		hscale, vscale;
		float		*out = scratch;
		int		i;

		if (lerpdata->pose1 >= hdr->nummorphposes || lerpdata->pose2 >= hdr->nummorphposes)
			return NULL;

		//the trivertx array is in vbo vertex order, so no indirection
		v1 = verts + (size_t)lerpdata->pose1 * numverts;
		v2 = verts + (size_t)lerpdata->pose2 * numverts;
		...
		for (i = 0; i < numverts; i++)
		{
			int j;
			for (j = 0; j < 3; j++)
				*out++ = (v1[i].v[j] + (v2[i].v[j] - v1[i].v[j]) * blend) * hdr->scale[j]
						+ hdr->scale_origin[j];

			if (withnormals)
			{	//lerp the two quantised normals and renormalise, so the
				//shading turns with the animation instead of snapping
				const float *n1 = r_avertexnormals[v1[i].lightnormalindex];
				const float *n2 = r_avertexnormals[v2[i].lightnormalindex];
				vec3_t n;
				float len;

				for (j = 0; j < 3; j++)
					n[j] = n1[j] + (n2[j] - n1[j]) * blend;
...
void R_DrawAliasModel (entity_t *e)
{
	...
	for(surf=0;;surf++)
	{
		...
		{
			/*
				One indexed draw, either from the model's immutable
				per-pose buffer -- where the same pose hashes identically
				every time it appears -- or from vertices interpolated
				this frame, which look right and hash differently every
				frame. d3d9_lerpmodels picks; see r_world.c.

				Lighting is skipped entirely -- Remix supplies it.
			*/
			if (e->eflags & EFLAGS_VIEWMODEL)
			{
				extern cvar_t d3d9_viewmodel_dist;
				vec3_t vmorg;

				/*
					Do NOT swap the view matrix the way the GL path does.
					Remix derives world positions from world x view, so a draw
					whose view matrix has no camera in it lands somewhere
					arbitrary in the level. Carry the gun's offset out to the
					camera in world space instead and leave the view alone.

					Then push it forward off the camera itself: origin[0] is
					the forward axis in the viewmodel's own space, and geometry
					sitting exactly on the ray origin is not traceable.
				*/
				VectorCopy (lerpdata.origin, vmorg);
				vmorg[0] += d3d9_viewmodel_dist.value;

				D3D9_SetViewmodelTransform (vmorg, lerpdata.angles,
								r_refdef.vieworg, r_refdef.viewangles);
			}
			else
				D3D9_SetAliasTransform (lerpdata.origin, lerpdata.angles);

			GL_Bind (tex.base);

			/*
				Between keyframes rather than on one, when asked for.
				Costs the stable per-pose hash -- see d3d9_lerpmodels --
				so a keyframe that is not being blended anyway still
				goes through the static buffer below.
			*/
			lerping = d3d9_lerpmodels.value && !e->model->d3dskel &&
					lerpdata.blend > 0 && lerpdata.pose1 != lerpdata.pose2;

			//developer 2: one line per model submitted, to tell a gap in what
			//the engine sends from a gap in what the renderer shows
			if (developer.value >= 2)
				Con_DPrintf ("f%d %s pose %d blend %.2f %s @ %.0f %.0f %.0f\n", host_framecount,
						e->model->name, lerpdata.pose1, lerpdata.blend,
						lerping ? "lerped" : "keyframe",
						lerpdata.origin[0], lerpdata.origin[1], lerpdata.origin[2]);

			if (lerping)
			{
				int		lerpverts;
				qboolean	withnormals = d3d9_modelnormals.value ? true : false;
				const float	*lerpverts_p = R_BuildLerpedAliasVerts (paliashdr, &lerpdata, &lerpverts, withnormals);

				if (lerpverts_p)
				{
					D3D9_DrawAliasLerped (lerpverts_p, lerpverts,
								(const unsigned short *)((byte *)paliashdr + paliashdr->indexes),
								paliashdr->numindexes, withnormals);
			...
			{
				/*
					Hardware skinned (md5/iqm, small enough skeleton).
					The buffer is the bind pose and never changes; the
					animation goes down as a palette of bone matrices and
					Remix skins it on the GPU, so one hash covers every
					frame of every animation.

					lerpdata.bonestate is already lerped between
					keyframes and multiplied by the inverse bind pose,
					and none of that touches vertex data -- r_lerpmodels
					can stay on for these.
				*/
				if (lerpdata.bonestate)
					D3D9_DrawSkeletal (e->model->d3dskel, (const float *)lerpdata.bonestate,
								paliashdr->numbones);
			}
			else
				//everything else, including md5 models whose skeleton
				//did not fit the palette and were skinned pose by pose
				//at load, is one static pose out of a static buffer
				D3D9_DrawAlias (e->model->d3dalias, lerpdata.pose1,
						paliashdr->numverts_vbo, paliashdr->numindexes);
