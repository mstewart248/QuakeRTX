...
static void TexMgr_Anisotropy_f (cvar_t *var)
{
	...
	{	//filtering is sampler state per texture here, not a GL call per bind
		gltexture_t *glt;
		for (glt = active_gltextures; glt; glt = glt->next)
...
gltexture_t *TexMgr_NewTexture (void)
{
	...
	if (!free_gltextures)
	{
		...
		/*
			Zero the block. Vanilla gets away without it because every field
			it reads is written by TexMgr_LoadImage first, but d3dtex is
			tested before it is assigned -- TexMgr_LoadImage32 destroys any
			texture already there -- so on a fresh block that test reads
			uninitialised heap and D3D9_DestroyTexture releases a garbage
			pointer. Recycled entries are clean (GL_DeleteTexture clears it);
			it is only ever the newly malloced ones that bite, which is why
			this needs a texture count past the first 64 to show up at all.
		*/
		memset (free_gltextures, 0, newtexturecount * sizeof(gltexture_t));
...
static void TexMgr_LoadImage32 (gltexture_t *glt, unsigned *data)
{
	...
	if (D3D9_Active())
	{	//D3D9 needs the level count when the texture is created, so work it
		//out up front rather than growing the chain a level at a time.
		int levels = 1;
		...
		if (glt->d3dtex)
			D3D9_DestroyTexture (glt->d3dtex);
		glt->d3dtex = D3D9_CreateTexture (glt->width, glt->height, levels, (glt->flags & TEXPREF_ALPHA) != 0);

		D3D9_UploadLevel (glt->d3dtex, 0, glt->width, glt->height, data);

		mipwidth = glt->width;
		mipheight = glt->height;
		for (miplevel = 1; miplevel < levels; miplevel++)
		{
			if (mipwidth > 1)
			{
			...
			D3D9_UploadLevel (glt->d3dtex, miplevel, mipwidth, mipheight, data);
		}
...
static void TexMgr_LoadImage8 (gltexture_t *glt, byte *data)
{
	...
	/*
		Under -d3d9 the nobright split is bypassed (see the TEXPREF_NOBRIGHT
		test below). QuakeSpasm normally blacks out palette indices 224-255 in
		the base texture and adds the glow back in a separate additive pass,
		which is why light fixtures otherwise render with black holes where
		their lit parts should be.

		We keep the fullbright pixels in the albedo instead of adding a second
		pass, because a second pass would resubmit the same surfaces as extra
		geometry and extra hashes for Remix to track. One draw per surface with
		correct albedo is what a path tracer wants; the actual glow comes from
		lights placed in Remix, not from the texture.
	*/
	...
	else if (glt->flags & TEXPREF_NOBRIGHT && gl_fullbrights.value && !D3D9_Active())
	{
		if (glt->flags & TEXPREF_ALPHA)
...
void GL_Bind (gltexture_t *texture)
{
	...
	if (D3D9_Active())
	{	//D3D9_BindTexture does its own redundancy filtering, and it also has to
		//reapply sampler state, so don't gate it on the GL binding cache.
		D3D9_BindTexture (currenttarget - GL_TEXTURE0_ARB, texture->d3dtex);
...
static void GL_DeleteTexture (gltexture_t *texture)
{
	...
	if (texture->d3dtex)
	{
		D3D9_DestroyTexture (texture->d3dtex);
		texture->d3dtex = NULL;
	}
