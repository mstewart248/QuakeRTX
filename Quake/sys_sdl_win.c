...
/*
================
Sys_CrashHandler -- -crashlog

Writes a symbolised stack trace to crash.log when the process faults. Only
installed on request: it exists for chasing the kind of fault whose reported
address is in no module at all, where the return chain is the only evidence
of who got there.

Frames come from walking EBP, which the 32 bit build keeps, and names from
dbghelp against the .pdb next to the exe -- loaded dynamically so a missing
dbghelp costs nothing.
================
*/
typedef struct
{
...
typedef BOOL (WINAPI *SymInitialize_f) (HANDLE, PCSTR, BOOL);
typedef BOOL (WINAPI *SymFromAddr_f) (HANDLE, DWORD64, PDWORD64, void *);
typedef DWORD (WINAPI *SymSetOptions_f) (DWORD);

static void Sys_CrashLogAddr (FILE *f, SymFromAddr_f symfromaddr, HANDLE proc, DWORD addr, int depth)
{
	crash_syminfo_t	si;
	DWORD64		disp = 0;
	...
	mod[0] = 0;
	if (VirtualQuery ((void *)(uintptr_t)addr, &mbi, sizeof(mbi)) && mbi.AllocationBase)
		GetModuleFileNameA ((HMODULE)mbi.AllocationBase, mod, sizeof(mod));

	memset (&si, 0, sizeof(si));
	si.SizeOfStruct = 88;	//sizeof(SYMBOL_INFO), name array excluded
	si.MaxNameLen = sizeof(si.Name) - 1;

	if (symfromaddr && symfromaddr (proc, addr, &disp, &si) && si.Name[0])
		fprintf (f, "  [%2d] 0x%08lX  %s +0x%lX  (%s)\n", depth, (unsigned long)addr,
				si.Name, (unsigned long)disp, mod[0] ? mod : "?");
	else
		fprintf (f, "  [%2d] 0x%08lX  <no symbol>  (%s)\n", depth, (unsigned long)addr,
				mod[0] ? mod : "NOT IN ANY MODULE");
}

static LONG WINAPI Sys_CrashHandler (EXCEPTION_POINTERS *ep)
{
	FILE		*f;
	HMODULE		dbghelp;
	SymInitialize_f	syminit = NULL;
	SymFromAddr_f	symfromaddr = NULL;
	SymSetOptions_f	symsetopts = NULL;
	...
	fprintf (f, "exception 0x%08lX at 0x%08lX\n",
			(unsigned long)ep->ExceptionRecord->ExceptionCode,
			(unsigned long)(uintptr_t)ep->ExceptionRecord->ExceptionAddress);
	if (ep->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION &&
	    ep->ExceptionRecord->NumberParameters >= 2)
		fprintf (f, "  %s address 0x%08lX\n",
	...
	if (dbghelp)
	{
		syminit = (SymInitialize_f) GetProcAddress (dbghelp, "SymInitialize");
		symfromaddr = (SymFromAddr_f) GetProcAddress (dbghelp, "SymFromAddr");
		symsetopts = (SymSetOptions_f) GetProcAddress (dbghelp, "SymSetOptions");
		if (symsetopts) symsetopts (0x00000004 | 0x00000200);	//UNDNAME|LOAD_LINES
		if (syminit) syminit (proc, NULL, TRUE);
	}

	fprintf (f, "\nfaulting instruction:\n");
	Sys_CrashLogAddr (f, symfromaddr, proc, ep->ContextRecord->Eip, 0);

	fprintf (f, "\nreturn chain (EBP walk):\n");
	frame = (DWORD *)(uintptr_t)ep->ContextRecord->Ebp;
	for (depth = 1; depth <= 40 && frame; depth++)
	{
		DWORD next, ret;

		if (IsBadReadPtr (frame, sizeof(DWORD) * 2))
			break;
		next = frame[0];
		ret  = frame[1];
		if (!ret)
			break;
		Sys_CrashLogAddr (f, symfromaddr, proc, ret, depth);
		if (next <= (DWORD)(uintptr_t)frame)
			break;	//stack grows down: anything else is a smashed frame
		frame = (DWORD *)(uintptr_t)next;
	}
...
#else	//SDL 1.2 has no performance counter; millisecond ticks are all we get
