...
/*
	Baked lightmaps are off by default under -d3d9.

	RTX Remix path traces the scene and supplies its own lighting, so feeding
	it surfaces that already have Quake's lightmaps modulated in double-lights
	everything and leaves baked shadows burnt into the albedo. Remix wants flat
	albedo. Set this to 1 if you want the classic lit look instead -- useful for
	checking the renderer against the GL path, less useful for remastering.
*/
cvar_t d3d9_lightmaps = {"d3d9_lightmaps", "0", CVAR_ARCHIVE};

/*
	Brightness multiplier for map lights handed to Remix.

	Quake's "light" key is a radius in map units, not a physical intensity, and
	the lights are submitted with inverse-square attenuation. Radiance at the
	surface therefore falls off as 1/d^2 over distances of hundreds of units,
	so the raw value needs scaling by roughly radius^2 to produce anything a
	path tracer can see. This is the knob for tuning that against a real map.
*/
cvar_t d3d9_lightscale = {"d3d9_lightscale", "1", CVAR_ARCHIVE};

/*
	How far in front of the eye to push the weapon model, in map units.

	Quake puts the viewmodel entity exactly at the camera and relies on the
	near plane to make that work. A path tracer cannot: primary rays start at
	the camera, so geometry sitting on the ray origin is degenerate and Remix
	simply does not trace it. Moving the gun forward turns it into ordinary
	world geometry that gets traced, lit and hashed like anything else.

	The cost is that it is now really out there in the world: it will clip
	into walls when you stand close to one, and it looks slightly smaller and
	further away than the original. 0 restores the exact original placement
	(and, under Remix, an invisible gun).
*/
cvar_t d3d9_viewmodel_dist = {"d3d9_viewmodel_dist", "16", CVAR_ARCHIVE};

/*
	Hardware skinning for skeletal (md5) models.

	Fixed function can blend four bone matrices per vertex out of a palette,
	which would let a whole animated model live in one static buffer and so
	carry one geometry hash for every frame it has -- ideal for replacement
	work. In practice nothing usable supports it: the NVIDIA D3D9 driver
	reports a palette of one matrix, and while the Remix runtime reports nine,
	feeding it blended geometry is not something to enable by default.

	Off, every skeletal model is skinned pose by pose at load instead, which
	is slower to load and heavier on memory but works everywhere and keeps the
	same per-pose hash stability the .mdl path has. Takes effect on map load.
*/
cvar_t d3d9_gpuskinning = {"d3d9_gpuskinning", "0", CVAR_ARCHIVE};

/*
	Draw models between keyframes instead of on them.

	Quake animates at 10 frames a second and relies on the engine to fill in
	the gaps -- r_lerpmodels does it for the GL path, blending two poses for a
	.mdl and the bone matrices for an md5. The buffered path here cannot: it
	picks one of the poses it baked at load, so animation steps at 10Hz no
	matter what the framerate is, which reads as jerky on everything and
	especially on the rerelease models.

	1 (default): submit the interpolated vertices the engine has already
	computed. Smooth, at the price of geometry that differs every frame, so
	Remix cannot anchor a replacement to a mesh hash. Anchor on an asset hash
	without positions instead (rtx.geometryAssetHashRuleString), which is what
	an animated game needs regardless.

	0: keyframes only, out of the immutable per-pose buffers. Jerky, but every
	pose presents the same vertices every time and hashes accordingly. This is
	the mode to capture and author replacements in.

	Either way a model that is not between poses at that instant -- anything
	unanimated, and every frame when r_lerpmodels is 0 -- takes the buffered
	path, so those hashes stay stable in both modes.
*/
cvar_t d3d9_lerpmodels = {"d3d9_lerpmodels", "1", CVAR_ARCHIVE};

/*
	Send a normal per vertex with alias models.

	This is the Gouraud-vs-flat question, moved to where it lives under a path
	tracer. Nothing here shades anything -- Remix does the lighting -- but it
	can only shade smoothly if it is told which way each vertex faces. With no
	normal in the vertex format it falls back to the triangle's own, and a
	model lit by a real light comes out visibly faceted.

	The data has always been there: a .mdl stores a lightnormalindex per
	vertex (the same 162 normals gl_smoothmodels uses for its shading dots),
	and md5 has real per-vertex normals which get skinned along with the
	positions.

	1 (default): xyz + normal + st. Costs 12 bytes a vertex.
	0: xyz + st, as before, and Remix faceted the models for you.

	Changing it changes the vertex layout, which Remix hashes -- so every
	alias geometry hash moves with it. Pick one before authoring replacements.
	Takes effect on map load for keyframed models; interpolated draws build
	their vertices per frame and switch immediately.
*/
cvar_t d3d9_modelnormals = {"d3d9_modelnormals", "1", CVAR_ARCHIVE};

/*
	Who draws the sky.

	0 (default): gl_sky.c draws the original's scrolling parallaxing cloud
	layers. This is the combination that works under Remix, with the sky
	texture tagged as a world-space UI texture in the Toolkit.

	It depends on DrawGLPoly staying unshimmed in r_brush.c -- see the note
	there. The sky brush surfaces must NOT be drawn; only the cloud layers
	should reach Remix. Shim them and the traced image dies whenever sky is
	in view.

	Known limitation: the scroll lives in the texcoords (Sky_GetTexCoord folds
	cl.time into s/t) while the vertex positions are re-centred on the camera
	each frame, so Remix reuses its cached geometry while you stand still and
	the clouds appear to stop until you move again.

	1: the world pass draws sky surfaces flat with the sky texture bound.
	Static, textured, stably hashed and taggable for Remix to draw its own sky
	through -- but no scrolling and no parallax.
*/
cvar_t d3d9_skytextured = {"d3d9_skytextured", "0", CVAR_ARCHIVE};

/*
	How much of the level to hand Remix.

	A rasteriser only has to submit what ends up on screen, so Quake throws
	away everything outside the view frustum and every surface facing away from
	the eye. A path tracer needs the opposite: geometry that is off screen still
	has to exist, or a reflection has nothing to reflect, a bounce has nothing
	to bounce off, and an emissive surface behind you contributes no light.
	Whatever Quake culls is simply absent from Remix's BVH.

	0: Quake's own culling. Right for the raster image, wrong for tracing.

	1 (default): no frustum culling. The whole PVS is submitted, so everything
	potentially visible from where the player is standing is in the scene no
	matter which way they are facing. This also covers entities and static
	entities -- torches, flames, monsters -- since R_CullModelForEntity and the
	efrag walk both go through R_CullBox.

	2: also no backface culling, so surfaces whose plane faces away from the eye
	are submitted. The rasteriser still drops them (gl_cull leaves D3DCULL_CCW
	on) so the on-screen image is unchanged, but a reflection ray that carries
	into the next room can now hit its walls from the inside. Roughly doubles
	the world draw calls.

	Note this widens the frustum, not the PVS. A room the engine considers
	unreachable from the current leaf stays absent; r_novis 1 submits the whole
	map if you need that, at a real cost in draw calls.
*/
cvar_t d3d9_nocull = {"d3d9_nocull", "1", CVAR_ARCHIVE};

/*
	How far past the player's own PVS to submit world geometry.

	d3d9_nocull only widens the frustum; the PVS is still the set of leaves
	potentially visible from the leaf the player is standing in, and it changes
	as they walk. Every change adds and removes geometry, which is what makes
	shadows and reflections shift as you move.

	Each step here unions in the PVS of every leaf already in the set -- so 1
	gives "everything visible from anywhere the player can currently see",
	2 goes a room further, and so on. It is recomputed only when the player
	crosses into a new leaf, so the per-frame cost is nil; what it costs is
	the extra surfaces submitted every frame.

	0 turns it off. r_novis 1 remains the sledgehammer: the entire map, no
	visibility at all.
*/
cvar_t d3d9_pvsexpand = {"d3d9_pvsexpand", "1", CVAR_ARCHIVE};

/*
	Cap on how many map lights are handed to Remix at once.

	0 means use whatever the runtime will accept, which D3D9_ProbeLightCapacity
	measures at startup. Quake's fixed-function caps say 8, but that limit is
	about lighting a vertex, and nothing here lights a vertex -- the lights
	exist only for Remix to convert. Lower it if the light count turns out to
	cost more than it is worth.
*/
cvar_t d3d9_lightlimit = {"d3d9_lightlimit", "0", CVAR_ARCHIVE};

/*
	Cycle the map's lights through the light slots across the frame.

	D3D9 fixed function holds 8 lights at a time. That is a hard limit: the
	runtime returns success from LightEnable well past it, but only the first 8
	are really there, which is why submitting all 267 at once produced 8 lights
	clustered wherever the first 8 happened to be.

	Remix does not take one snapshot per frame though -- it accumulates the
	lights it sees across every draw call. That is why games that swap their 8
	slots per object get all of their lights captured. So do the same: rotate a
	fresh group of 8 in before each batched world draw, and over one frame every
	light in the map has been live.

	0 falls back to the nearest 8, which is all a single snapshot can carry.
*/
cvar_t d3d9_lightcycle = {"d3d9_lightcycle", "1", CVAR_ARCHIVE};

//lights live at once. 0 uses the D3DCAPS9 figure, which is the honest one.
cvar_t d3d9_lightgroup = {"d3d9_lightgroup", "0", CVAR_ARCHIVE};

/*
	Submit the whole static world as a few dozen batched draws.

	Quake hands the GPU one draw per visible surface, which is right for a
	rasteriser and ruinous here: under Remix every call crosses a process
	boundary, and that cost -- not the geometry -- is what made submitting
	anything past the view frustum unaffordable. Pre-triangulating the world
	into one index run per texture at map load turns thousands of draws into
	roughly one per world texture, cheap enough to simply draw the entire level
	every frame.

	Which is the real fix for a path tracer. No PVS, no frustum, no visible set
	that changes as the player walks, so nothing pops in or out of the BVH and
	the geometry hashes are identical every frame for the life of the map.

	Remix sees one mesh per texture rather than one per surface. Material
	replacement by texture hash is unaffected; replacing an individual wall as
	its own mesh is not possible while this is on.

	0 falls back to the per-surface path, which is also used regardless for
	brush model entities and whenever d3d9_lightmaps is set.
*/
cvar_t d3d9_worldbatch = {"d3d9_worldbatch", "1", CVAR_ARCHIVE};

//ceiling on the submit arrays, independent of what the device reports
#define D3D9_MAXSUBMITLIGHTS	1024
...
qboolean R_BackFaceCull (msurface_t *surf)
{
	...
	//d3d9_nocull 2: a surface facing away from the eye can still be the far
	//side of a reflection, so keep it -- see the cvar comment above
	if (r_nocull >= 2)
		return false;
...
Unions in the PVS of every leaf already in the set, "passes" times over. One
pass answers "what could I see from anywhere I can see from here", which is
the set that stops changing as the player walks around a single room.

Cached against the view leaf, so this only runs when the player crosses a leaf
boundary rather than every frame.
===============
*/
static byte	*r_pvsexpand_out = NULL;	//accumulated result, returned to the caller
static byte	*r_pvsexpand_src = NULL;	//snapshot to iterate while out is being written
static int	r_pvsexpand_capacity = 0;
static qmodel_t	*r_pvsexpand_model = NULL;
static mleaf_t	*r_pvsexpand_leaf = NULL;
static int	r_pvsexpand_passes = 0;

void R_PVSExpandNewMap (void)
{
	r_pvsexpand_model = NULL;
	r_pvsexpand_leaf = NULL;
	r_pvsexpand_passes = 0;
}

static byte *R_ExpandPVS (byte *vis, qmodel_t *model, int passes)
{
	int	rowbytes = (model->numleafs + 7) >> 3;
	int	i, b, pass;

	if (passes < 1 || rowbytes < 1)
		return vis;

	if (r_pvsexpand_out && r_pvsexpand_model == model &&
	    r_pvsexpand_leaf == r_viewleaf && r_pvsexpand_passes == passes)
		return r_pvsexpand_out;

	if (r_pvsexpand_capacity < rowbytes)
	{
		byte *a = (byte *) realloc (r_pvsexpand_out, rowbytes);
		byte *b2 = (byte *) realloc (r_pvsexpand_src, rowbytes);
		if (a) r_pvsexpand_out = a;
		if (b2) r_pvsexpand_src = b2;
		if (!a || !b2)
			return vis;
		r_pvsexpand_capacity = rowbytes;
	}

	/*
		vis points at the texture manager's shared decompression buffer, which
		Mod_LeafPVS below overwrites on every call. Copy it out first, and keep
		iterating over a snapshot so a leaf added by this pass does not get
		expanded again until the next one.
	*/
	memcpy (r_pvsexpand_out, vis, rowbytes);

	for (pass = 0; pass < passes; pass++)
	{
		memcpy (r_pvsexpand_src, r_pvsexpand_out, rowbytes);

		for (i = 0; i < model->numleafs; i++)
		{
			byte *leafvis;

			if (!(r_pvsexpand_src[i>>3] & (1u << (i&7))))
				continue;

			//bit i is leafs[i+1]; leaf 0 is the solid leaf and has no vis
			leafvis = Mod_LeafPVS (&model->leafs[i+1], model);
			for (b = 0; b < rowbytes; b++)
				r_pvsexpand_out[b] |= leafvis[b];
		}
	}

	r_pvsexpand_model = model;
	r_pvsexpand_leaf = r_viewleaf;
	r_pvsexpand_passes = passes;
...
void R_MarkSurfaces (void)
{
	...
	/*
		Widen it before anything consumes it. With d3d9_worldbatch on this no
		longer affects world geometry at all -- the whole map is drawn either
		way -- but entities still come from the PVS, and the static ones are
		exactly the torches whose lights were popping in.
	*/
	if (r_nocull && d3d9_pvsexpand.value > 0 && !r_novis.value)
		vis = R_ExpandPVS (vis, cl.worldmodel, (int)d3d9_pvsexpand.value);
...
MAP LIGHTS -> D3D9 FIXED FUNCTION LIGHTS

Quake's lights live in the BSP entity lump and never reach the renderer, so
Remix has no way to know they exist. Parsing them out and submitting them as
D3D9 point lights gives Remix real light sources to convert, hash and let you
replace in the Toolkit.

=============================================================================
*/

#define D3D9_MAX_MAPLIGHTS	4096
...
static d3d9maplight_t	d3d9_maplights[D3D9_MAX_MAPLIGHTS];
static int		d3d9_nummaplights = 0;
static qmodel_t		*d3d9_lightsparsedfor = NULL;

/*
================
R_D3D9_ParseMapLights
================
*/
static void R_D3D9_ParseMapLights (void)
{
	const char	*data;
	...
	d3d9_nummaplights = 0;
	d3d9_lightsparsedfor = cl.worldmodel;
	...
	while (1)
	{
		...
		memset (&l, 0, sizeof(l));
		l.radius = 300.0f;			//Quake's default "light" value
		l.colour[0] = l.colour[1] = l.colour[2] = 1.0f;
		islight = false;
		...
		while (1)
		{
			...
			{	//light, light_fluoro, light_torch_small_walltorch, ...
			...
			{
				sscanf (value, "%f %f %f", &l.colour[0], &l.colour[1], &l.colour[2]);
				//some maps write 0-255, others 0-1
				if (l.colour[0] > 1.0f || l.colour[1] > 1.0f || l.colour[2] > 1.0f)
				{
					l.colour[0] /= 255.0f;
					l.colour[1] /= 255.0f;
					l.colour[2] /= 255.0f;
		...
		if (islight && l.radius > 0 && d3d9_nummaplights < D3D9_MAX_MAPLIGHTS)
			d3d9_maplights[d3d9_nummaplights++] = l;

		if (!data)
			break;
	}

	Con_Printf ("D3D9: %d map lights parsed from entity lump\n", d3d9_nummaplights);
}

/*
================
R_D3D9_SubmitLightSet

Loads the given map lights into slots 0..n-1.
================
*/
static void R_D3D9_SubmitLightSet (const int *idx, int n)
{
	static float	origins[D3D9_MAXSUBMITLIGHTS*3], radii[D3D9_MAXSUBMITLIGHTS], colours[D3D9_MAXSUBMITLIGHTS*3];
	int		i;

	if (n <= 0)
		return;
	if (n > D3D9_MAXSUBMITLIGHTS)
		n = D3D9_MAXSUBMITLIGHTS;

	for (i = 0; i < n; i++)
	{
		d3d9maplight_t	*l = &d3d9_maplights[idx[i]];
		float		intensity;

		VectorCopy (l->origin, &origins[i*3]);

		//radius^2 to counteract the inverse-square falloff over map distances
		intensity = l->radius * l->radius * 0.01f * d3d9_lightscale.value;

		colours[i*3+0] = l->colour[0] * intensity;
		colours[i*3+1] = l->colour[1] * intensity;
		colours[i*3+2] = l->colour[2] * intensity;

		//generous range so a light still reaches the room it lives in
		radii[i] = l->radius * 4.0f;
	}

	D3D9_SubmitLights (origins, radii, colours, n);
}

/*
================
R_D3D9_LightGroupSize

How many lights are live at once. This is the D3DCAPS9 figure, and unlike the
count the runtime will let us Enable, it is the one that is actually honoured.
================
*/
static int R_D3D9_LightGroupSize (void)
{
	int n = (int)d3d9_lightgroup.value;

	if (n <= 0)
	{
		n = D3D9_CapsActiveLights ();
		if (n <= 0)
			n = 8;
	}
	if (d3d9_lightlimit.value > 0 && n > (int)d3d9_lightlimit.value)
		n = (int)d3d9_lightlimit.value;
	if (n > D3D9_MAXSUBMITLIGHTS)
		n = D3D9_MAXSUBMITLIGHTS;
...
/*
================
R_D3D9_CycleMapLights

Rotates the next group of map lights into the light slots, called between
world batches so that over one frame every light in the map has been live for
at least one draw call.

The fixed function pipeline holds 8 lights at a time and no amount of asking
gets more -- LightEnable happily returns success past that and the extra
lights are simply not there. But Remix builds its light list from what it sees
across the whole frame, not from one snapshot, which is why games that swap
their 8 lights per object get all of their lights captured. Doing the same
here is what makes a map's worth of lights reachable through an 8 light API.

Costs 2 calls per light per frame; with the world down to a few dozen batched
draws there is room for it.
================
*/
void R_D3D9_CycleMapLights (int batch)
{
	int	idx[D3D9_MAXSUBMITLIGHTS];
	int	i, first, size, groups;

	if (!D3D9_Active() || !d3d9_lightcycle.value || d3d9_nummaplights <= 0)
		return;

	size = R_D3D9_LightGroupSize ();
	groups = (d3d9_nummaplights + size - 1) / size;
	if (groups <= 1)
		return;		//they all fit at once; nothing to rotate

	first = (batch % groups) * size;
	if (first + size > d3d9_nummaplights)
		size = d3d9_nummaplights - first;

	for (i = 0; i < size; i++)
		idx[i] = first + i;

	R_D3D9_SubmitLightSet (idx, size);
}

/*
================
R_D3D9_SubmitMapLights

Called once at view setup. With cycling on this only seeds the first group and
the batch loop carries it round; with cycling off it falls back to the nearest
few, which is all an 8 light API can otherwise show.
================
*/
void R_D3D9_SubmitMapLights (void)
{
	int	order[D3D9_MAXSUBMITLIGHTS];
	float	bestdist[D3D9_MAXSUBMITLIGHTS];
	int	i, j, k, count, maxlights;

	if (!D3D9_Active())
		return;

	if (cl.worldmodel != d3d9_lightsparsedfor)
		R_D3D9_ParseMapLights ();

	if (d3d9_nummaplights <= 0)
		return;

	maxlights = R_D3D9_LightGroupSize ();

	if (d3d9_lightcycle.value)
	{
		static int reported = -1;
		int groups = (d3d9_nummaplights + maxlights - 1) / maxlights;

		if (reported != d3d9_nummaplights)
		{
			reported = d3d9_nummaplights;
			Con_Printf ("D3D9: %d map lights, %d per group, %d groups per frame\n",
					d3d9_nummaplights, maxlights, groups);
		}

		R_D3D9_CycleMapLights (0);
		return;
	}

	count = 0;
	for (i = 0; i < d3d9_nummaplights; i++)
	{
		vec3_t	diff;
		float	d;

		VectorSubtract (d3d9_maplights[i].origin, r_refdef.vieworg, diff);
		d = DotProduct (diff, diff);	//squared is fine, we only order by it
		...
		for (k = q_min(count, maxlights - 1); k > j; k--)
		{
	...
	/*
		Re-sort the chosen set into map order before submitting, so a light
		keeps the same slot for as long as it stays in the set rather than
		shuffling down one every time a nearer light appears.
	*/
	for (i = 1; i < count; i++)
	{
		int cur = order[i];
		for (j = i; j > 0 && order[j-1] > cur; j--)
			order[j] = order[j-1];
		order[j] = cur;
	}

	R_D3D9_SubmitLightSet (order, count);
}

/*
=============================================================================

STATIC WORLD BATCHES

The whole static world, triangulated once at map load and grouped into one
index run per texture. See d3d9_backend.h on why the draw call count is the
...
typedef struct
{
	int	texture;	//index into worldmodel->textures
	int	firstindex;
	int	numindices;
	int	minvert;
	int	numverts;
...
static d3d9worldbatch_t	*d3d9_worldbatches = NULL;
static int		d3d9_numworldbatches = 0;
static qmodel_t		*d3d9_batchesbuiltfor = NULL;

/*
================
R_D3D9_FreeWorldBatches
================
*/
void R_D3D9_FreeWorldBatches (void)
{
	free (d3d9_worldbatches);
	d3d9_worldbatches = NULL;
	d3d9_numworldbatches = 0;
	d3d9_batchesbuiltfor = NULL;
	D3D9_DestroyWorldIndexBuffer ();
}

/*
================
R_D3D9_BuildWorldBatches

Called once per map, straight after the world vertex buffer is filled in
GL_BuildBModelVertexBuffer -- that is where every surface gets its
vbo_firstvert, which is all we need to triangulate.

Only the world submodel goes in. Brush submodels (doors, platforms, lifts)
share the same vertex buffer but move under their own matrix, so they stay on
the per-surface path.
================
*/
void R_D3D9_BuildWorldBatches (void)
{
	qmodel_t	*mod = cl.worldmodel;
	unsigned int	*indices;
	int		i, k, first, last, total, count;

	R_D3D9_FreeWorldBatches ();

	if (!D3D9_Active() || !mod || !mod->numtextures)
		return;

	first = mod->firstmodelsurface;
	last = first + mod->nummodelsurfaces;
	if (last > mod->numsurfaces)
		last = mod->numsurfaces;

	//worst case: every surface a fan of numedges-2 triangles
	total = 0;
	for (i = first; i < last; i++)
		if (mod->surfaces[i].numedges >= 3)
			total += (mod->surfaces[i].numedges - 2) * 3;

	if (total < 3)
		return;

	indices = (unsigned int *) malloc ((size_t)total * sizeof(*indices));
	d3d9_worldbatches = (d3d9worldbatch_t *) calloc (mod->numtextures, sizeof(*d3d9_worldbatches));
	if (!indices || !d3d9_worldbatches)
	{
		free (indices);
		R_D3D9_FreeWorldBatches ();
		return;
	...
	count = 0;	//indices written so far

	for (k = 0; k < mod->numtextures; k++)
	{
		texture_t		*t = mod->textures[k];
		d3d9worldbatch_t	*b;
		int			batchfirst = count;
		int			lo = mod->numsurfaces ? 0x7FFFFFFF : 0, hi = -1;
		...
		for (i = first; i < last; i++)
		{
			...
			if (s->texinfo->texture != t)
				continue;
			if (s->numedges < 3)
				continue;
			/*
				Sky is the baked cube's job now (gl_sky.c), and an untextured
				surface has nothing to bind. Water DOES belong here: it draws as
				ordinary flat static geometry in this backend, and under -d3d9
				R_DrawTextureChains_Water is inert GL calls, so there is no
				second pass to collide with.
			*/
			if (s->flags & (SURF_DRAWSKY | SURF_NOTEXTURE))
				continue;

			base = s->vbo_firstvert;

			for (e = 1; e <= s->numedges - 2; e++)
			{	//fan to list, matching D3DPT_TRIANGLEFAN's winding
				indices[count++] = (unsigned int)base;
				indices[count++] = (unsigned int)(base + e);
				indices[count++] = (unsigned int)(base + e + 1);
			}

			if (base < lo) lo = base;
			if (base + s->numedges - 1 > hi) hi = base + s->numedges - 1;
		}

		if (count == batchfirst)
			continue;	//no surfaces used this texture

		b = &d3d9_worldbatches[d3d9_numworldbatches++];
		b->texture = k;
		b->firstindex = batchfirst;
		b->numindices = count - batchfirst;
		b->minvert = lo;
		b->numverts = hi - lo + 1;
	}

	if (!d3d9_numworldbatches || !D3D9_CreateWorldIndexBuffer (indices, count))
	{
		free (indices);
		R_D3D9_FreeWorldBatches ();
		return;
	...
	free (indices);
	d3d9_batchesbuiltfor = mod;

	Con_Printf ("D3D9: world in %d batches, %d tris (was %d draws)\n",
			d3d9_numworldbatches, count / 3, last - first);
}

/*
================
R_DrawWorldBatches_D3D9

The entire static world, every frame, regardless of PVS or frustum. That is
the point: what is not submitted does not exist to the path tracer.
================
*/
static void R_DrawWorldBatches_D3D9 (void)
{
	int i;

	for (i = 0; i < d3d9_numworldbatches; i++)
	{
		d3d9worldbatch_t	*b = &d3d9_worldbatches[i];
		texture_t		*t = cl.worldmodel->textures[b->texture];

		if (!t)
			continue;

		/*
			Rotate the next group of map lights in before this batch goes out.
			Remix accumulates lights over the frame, so cycling them through the
			8 available slots is what gets a whole map's worth of lights across
			an API that only ever holds 8.
		*/
		R_D3D9_CycleMapLights (i);

		//the batch is fixed; only which frame of an animated texture is bound
		//changes from frame to frame
		GL_Bind (R_TextureAnimation(t, 0)->gltexture);

		D3D9_DrawWorldBatch (b->firstindex, b->numindices, b->minvert, b->numverts);
		rs_brushpasses++;
	}
}
...
/*
================
R_DrawTextureChains_D3D9

Every surface is a triangle fan drawn straight out of the immutable world
vertex buffer at its fixed offset. Nothing here writes vertex data, which is
what keeps geometry hashes stable frame to frame for Remix.
================
*/
static void R_DrawTextureChains_D3D9 (qmodel_t *model, entity_t *ent, texchain_t chain)
{
	int		i;
	msurface_t	*s;
	texture_t	*t;
	//NB: not "lightmaps" -- that is the global lightmap array
	qboolean	uselightmaps = (d3d9_lightmaps.value != 0);

	D3D9_BeginWorld (uselightmaps, gl_overbright.value != 0);

	/*
		The batched path draws the whole static world and ignores the chains
		entirely. It cannot serve the lightmapped path, which needs a per
		surface lightmap bind, and it only covers the world submodel -- brush
		model entities move and fall through to the loop below.
	*/
	if (model == cl.worldmodel && chain == chain_world && !uselightmaps &&
	    d3d9_worldbatch.value && d3d9_batchesbuiltfor == cl.worldmodel &&
	...
	for (i = 0; i < model->numtextures; i++)
	{
		...
		if (!t || !t->texturechains[chain])
			continue;
		/*
			Water draws as ordinary static geometry here, flat and unwarped.
			Quake's water warp is a per-frame vertex animation, and animated
			vertices rehash every frame, which is precisely what stops Remix
			anchoring a replacement material to a surface. Drawing the
			unsubdivided polygon straight out of the static buffer keeps the
			hash stable, and a remaster substitutes a real water material
			anyway.

			Sky is deliberately NOT drawn here. gl_sky.c owns it and produces
			the two scrolling, parallaxing layers the original has; drawing it
			flat here as well would both double-draw and lose that effect. Sky
			is one thing Remix cannot replace, so it has to look right in the
			raster output.

			Test SURF_DRAWSKY, not SURF_DRAWTILED: gl_model.c gives DRAWTILED to
			sky, to unlit water AND to untextured surfaces, so skipping on it
			drops water too and leaves those areas black.
		*/
		if (t->texturechains[chain]->flags & SURF_NOTEXTURE)
			continue;

		//sky: ours only when d3d9_skytextured is set, otherwise gl_sky.c's
		if ((t->texturechains[chain]->flags & SURF_DRAWSKY) && !d3d9_skytextured.value)
			continue;
		...
		for (s = t->texturechains[chain]; s; s = s->texturechain)
		{
			if (uselightmaps && s->lightmaptexturenum >= 0)
			{
			...
			D3D9_DrawWorldSurface (s->vbo_firstvert, s->numedges);
			rs_brushpasses++;
		}
	}
...
void R_DrawTextureChains (qmodel_t *model, entity_t *ent, texchain_t chain)
{
	...
	if (D3D9_Active())
	{
		R_DrawTextureChains_D3D9 (model, ent, chain);
		return;
	}
...
//	int type = 0;
	...
	else if (!gl_glsl_able || r_fullbright_cheatsafe || r_lightmap_cheatsafe || r_drawflat_cheatsafe)
	{	//without glsl there is no r_world_program and no vbo batching for the cache to feed, so the
		//legacy fixed-function chains are the only thing that can draw the scene -- crashes otherwise (-noglsl).
...
#else
			rscenecache.thread = SDL_CreateThread(RSceneCache_Thread, NULL);	//SDL 1.2 has no thread-name argument
#endif
			if (!rscenecache.thread)
			{
