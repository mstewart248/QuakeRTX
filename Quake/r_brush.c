...
/*
	Deliberately NOT including d3d9_imm.h here.

	DrawGLPoly/DrawGLTriangleFan below are what Sky_ProcessPoly uses to lay
	down the sky brush surfaces. Routing them through the shim makes them draw
	for real, and under Remix that puts untextured opaque geometry across every
	sky opening, which kills the traced image whenever sky is in view. Leaving
	them as unshimmed GL calls (no-ops with no context) is what lets the
	scrolling sky work under Remix.
*/
...
// instanced model
	...
	/*
		Doors, platforms and other movers are brush model entities: their
		geometry lives in the static world buffer and only this transform
		changes as they move. Without it they render forever at their spawn
		position. Note e->angles[0] was negated just above (Quake's own bug,
		reproduced here), and D3D9_SetAliasTransform negates pitch again, so
		the two match the GL path exactly.

		Keeping the mesh static and moving it by matrix is also what Remix
		wants -- one stable hash, animated as an instance.
	*/
	if (D3D9_Active())
		D3D9_SetAliasTransform (e->origin, e->angles);
...
void GL_BuildBModelVertexBuffer (void)
{
	...
	if (D3D9_Active())
		;	//no GL objects needed; the D3D9 buffer is created below
	else if (!(gl_vbo_able && gl_mtexable && gl_max_texture_units >= 3))
		return;
...
// build vertex array
	...
	if (D3D9_Active())
	{	//one immutable buffer for every world surface, written once here and
		//never touched again -- see d3d9_backend.h on why that matters.
		D3D9_CreateWorldBuffer (varray, (int)numverts);
		free (varray);

		//and the static index runs over it, now that every surface knows its
		//vbo_firstvert
