...
static void Mod_MD5Models_f (cvar_t *var);
...
cvar_t	r_md5models = {"r_md5models", "1", CVAR_ARCHIVE};	//the 2021 rerelease's skeletal models, when the game data has them
...
Models are resolved once, when the map precaches them, and every entity in the
world then holds a pointer to what was loaded. Swapping the format underneath
them is not something to do mid-map, so the change waits for the next load.
===============
*/
static void Mod_MD5Models_f (cvar_t *var)
{
	...
	if (cls.state == ca_connected)
		Con_Printf ("r_md5models: takes effect on the next map load\n");
}
...
The extension list Mod_LoadModel walks when looking for a replacement for a
requested model, in priority order.

r_md5models is authoritative for md5: it prepends the extension when set, and
removes it from the user's r_replacemodels list when clear, so one cvar
switches the rerelease models on and off whatever else is configured.
===============
*/
static const char *Mod_ReplacementExts (void)
{
	static char	exts[256];
	const char	*in;
	size_t		len = 0;
	...
	if (r_md5models.value)
		len = q_strlcpy (exts, "md5mesh", sizeof(exts));

	in = r_replacemodels.string;
	while ((in = COM_Parse(in)))
	{
		if (!strcmp (com_token, "md5mesh"))
			continue;	//already there, or explicitly disabled

		if (len)
			len = q_strlcat (exts, " ", sizeof(exts));
		len = q_strlcat (exts, com_token, sizeof(exts));
		if (len >= sizeof(exts))
			break;
	}
...
void Mod_Init (void)
{
	...
	Cvar_SetCallback (&r_md5models, Mod_MD5Models_f);
...
//
	...
	{
		const char *exts = Mod_ReplacementExts ();
		char *e;
